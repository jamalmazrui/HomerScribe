...
HomerDescribe. Nothing here is needed in order to run it.
...
- **HomerDescribe.cs** — everything: settings, the dialog, the pipeline, the
...
- `%LOCALAPPDATA%\HomerDescribe\HomerDescribe.ini` — the settings.
- `%LOCALAPPDATA%\HomerDescribe\work\<video>-<tag>\` — the resume record, the
...
`C:\Program Files\HomerDescribe`, which an ordinary user cannot write to — a
...
build reads it into `Version.cs`, and `HomerDescribe_setup.iss` reads the same
...
`createHomerDescribeRepo.cmd` is the one-time bootstrap that creates the
...
Most of the quality lives in three places in `HomerDescribe.cs`:
...
2. **The dialogue as context.** Descriptions are written without knowing what was
   ...
   would fix this.
3. **A character bank.** The context file is a text-only version of AutoAD II's
   idea; the paper's version attaches face exemplars, which names characters far
   more reliably.
4. **A dialog that carries every setting.** It carries the ones named for it; the
